...
import ImageUploader from '../../../../components/modulos/moduleDigital/sayan/ImageFrom';
  ...
  return (
    <section className='bg-[#0d617b]/50 min-h-screen pb-20'>
      ...
      <div className="p-10 ml-40 font-mono text-3xl hover:scale-110 duration-300">
        <button onClick={() => window.history.back()} className="flex items-center bg-[#b6d900]/60 px-16 py-3 text-white rounded-xl uppercase">
        <FiChevronsLeft className='mr-2 text-5xl' /> {/* Icono a la izquierda */}
          Atrás
        </button>
      </div>
        <Link href="/modulePdf/modulePdfDigital/sayanPdf/page" className={`flex p-10 font-mono ${!excelLoaded ? 'pointer-events-none' : ''}`}>
          <button className={`bg-[#b6d900]/60 px-10 py-3 text-white rounded-xl uppercase text-3xl mb-10 mt-10 mr-48 hover:scale-110 duration-300 ${!excelLoaded || buttonDisabled ? 'opacity-50' : ''}`} disabled={buttonDisabled}>

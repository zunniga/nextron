...
import ImageUploader from '../../../../components/modulos/modulePhysical/promas/ImageFrom';
...
const ExcelDataFrom = () => {
  ...
  return (
    <section className='bg-[#360B7F] min-h-screen pb-20'>
      ...
      <div className='flex justify-center items-center mt-5 gap-6 p-8 bg-[#B20076]'>
      ...
      <div className="p-10 ml-40 font-mono text-3xl hover:scale-110 duration-300">
        <button onClick={() => window.history.back()} className="flex items-center bg-[#680080] px-16 py-3 text-white rounded-xl uppercase">
        <FiChevronsLeft className='mr-2 text-5xl' /> {/* Icono a la izquierda */}
          Atrás
        </button>
      </div>
      <Link href="/modulePdf/modulePdfPhysical/promasPdf/page" className={`flex p-10 font-mono ${!excelLoaded ? 'pointer-events-none' : ''}`}>
          <button className={`bg-[#680080] px-10 py-3 text-white rounded-xl uppercase text-3xl mb-10 mt-10 mr-48 hover:scale-110 duration-300 ${!excelLoaded || buttonDisabled ? 'opacity-50' : ''}`} disabled={buttonDisabled}>
      ...
      <div className='max-w-[1500px] mx-auto grid grid-cols-2 p-4 border-4 border-[#FD4660] rounded-2xl'>
        <div className=''>

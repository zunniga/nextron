...
import ImageUploader from '../../../../components/modulos/modulePhysical/rizo/ImageFrom';
import Link from 'next/link';
import { RiFileExcel2Line } from "react-icons/ri";
import { FiChevronsLeft, FiChevronsRight } from "react-icons/fi";
import { PulseLoader } from "react-spinners";
import { ipcRenderer } from'electron';
import physicExcelData from '../../../../components/modulos/utils/excelPhysic';
import { ExcelDataPhysic } from '../../../../components/modulos/interface/interface';

const ExcelDataFrom = () => {
  const [numModules, setNumModules] = useState(1);
  const [imageFiles, setImageFiles] = useState<File[]>([]);
  const [excelFiles, setExcelFiles] = useState<File[]>(Array(numModules).fill(null));
  const [imagesAndExcel, setImagesAndExcel] = useState<{ image: File | null; imageId: number | null; excelData:ExcelDataPhysic | null }[]>([]);
  const [excelData, setExcelData] = useState<ExcelDataPhysic | null>(null);
  ...
  useEffect(() => {
    const updateImagesAndExcel = async () => {
      const updatedImagesAndExcel = await Promise.all(excelFiles.map(async (file, index) => ({
        ...
        image: index < imageFiles.length ? imageFiles[index] : null,
        excelData: await physicExcelData(file),
      })));
      setImagesAndExcel(updatedImagesAndExcel);
     setTimeout(() => {
  ...
  return (
    <section className='bg-[#1C004C]/70 min-h-screen pb-20'>
      ...
      <div className='flex justify-center items-center mt-5 gap-6 p-8 bg-[#6D0083]'>
      ...
      <div className="p-10 ml-40 font-mono text-3xl hover:scale-110 duration-300">
        <button onClick={() => window.history.back()} className="flex items-center bg-[#6D0083]/70 px-16 py-3 text-white rounded-xl uppercase">
        <FiChevronsLeft className='mr-2 text-5xl' />
          Atrás
        </button>
      </div>
      <Link href="/modulePdf/modulePdfPhysical/rizoPdf/page" className={`flex p-10 font-mono ${!excelLoaded ? 'pointer-events-none' : ''}`}>
          <button className={`bg-[#6D0083]/70 px-10 py-3 text-white rounded-xl uppercase text-3xl mb-10 mt-10 mr-48 hover:scale-110 duration-300 ${!excelLoaded || buttonDisabled ? 'opacity-50' : ''}`} disabled={buttonDisabled}>
            <span className="flex items-center justify-center">
              {nextButtonText === "Siguiente" ? (
                <>
                  <span>{nextButtonText}</span>
                  <FiChevronsRight className="ml-2 text-5xl" />
                </>
              ) : (
                <>
                  <span>Dibujando</span>
                  <PulseLoader color="#ffff" size={10} className="ml-2 font-extrabold" />
                </>
              )}
            </span>
          </button>
      ...
      <div className='max-w-[1500px] mx-auto grid grid-cols-2 p-4 border-4 border-[#FF9D00] rounded-2xl'>
        <div className=''>
          ...
          {imagesAndExcel.length > 0 && (
            <div>
              <ImageUploader
                ...
                excelData={imagesAndExcel.map(item => item.excelData).filter(excelData => excelData !== null) as ExcelDataPhysic[]}
              />
            </div>
          )}
          </div>

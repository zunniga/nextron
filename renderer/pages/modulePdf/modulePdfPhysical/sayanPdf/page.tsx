...
import { openDatabase } from '../../../../components/modulos/modulePhysical/sayan/database/index';
        ...
        {imageGroups.map((group, groupIndex) => (
          <div key={groupIndex} className='flex justify-center'>
            <div className='gap-10 inline-flex items-center text-xl'>
              <h3 className='flex justify-between w-[1000px] border-4 border-[#b6d900]/60 text-gray-200 p-4 mt-5 items-center rounded-xl font-bold'>{group.name}
                <span className="text-sm text-gray-400 ml-1">
                  ({group.images.length} módul{group.images.length !== 0 ? 'os' : ''})
                </span>
              </h3>
              <button onClick={() => openModal(group)} className='w-40 p-4 mt-5 bg-[#12a9be] text-gray-200 rounded-xl font-mono hover:scale-110 duration-300'>Ver módulos</button>
              ...
              <div className='flex justify-center'>
                <div className="modal-buttons inline-flex mb-5">
                  <button
                    ...
                    className={`text-gray-200 mr-3 border-2 border-[#12a9be] hover:bg-[#12a9be] px-3 py-1 rounded-xl uppercase font-extrabold text-xl hover:scale-125 duration-300${currentImageIndex === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    &#60;
                  </button>
                  <div className="pagination flex font-extrabold">
                    {currentGroup.images.map((image, index) => (
                      <button
                        ...
                        className={`page-btn ${index === currentImageIndex ? 'bg-gray-300 text-[#12a9be] rounded-xl' : 'bg-[#12a9be] text-gray-200 hover:scale-125 duration-300 hover:bg-slate-200 hover:text-[#12a9be]'} px-4 py-2 rounded-xl`}>
                        {index + 1}
                      </button>
                    ))}
                  </div>
                  <button
                    ...
                    className={`text-gray-200 ml-3 border-2 border-[#12a9be] hover:bg-[#12a9be] px-3 py-1 rounded-xl uppercase font-extrabold text-xl hover:scale-125 duration-300${currentImageIndex === currentGroup.images.length - 1 ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    &#62;
                  </button>
                </div>
              </div>

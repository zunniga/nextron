...
import { openDatabase } from '../../../../components/modulos/moduleDigital/rizo/database/index';
...
const ImageExport = () => {
  ...
  useEffect(() => {
    const getImagesFromDB = async () => {
      try {
        ...
        const transaction = db.transaction(['ImagesRizo'], 'readonly');
        const objectStore = transaction.objectStore('ImagesRizo');
  ...
  const deleteImagesFromDB = async () => {
    try {
      setShowSpinner(true);
      const db = await openDatabase();
      const transaction = db.transaction(['ImagesRizo'], 'readwrite');
      const objectStore = transaction.objectStore('ImagesRizo');
...
const convertToPDFAndEmail = async (group: { name: string, images: File[] }) => {
  try {
    ...
    const emailContent = render(<RizoTemplate groupName={group.name} dataString={dataString}/>)
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: 'corporacionrizo@gmail.com',
        pass: 'pffgvmqlcmpbkbcd',
      }
    });
    const mailOptions = {
      from: 'corporacionrizo@gmail.com',
...
const handleEditEmailSave = (groupName: string) => {
  ...
  return (
    <div className="bg-[#1C004C] min-h-screen pb-1">
      <div className='ml-10'>
        <img
          src="/rizo.png"
          alt='rizo'
      ...
      <div className='flex justify-center items-center mt-5 gap-6 p-12 bg-[#6D0083]'>
  ...
  <div className='flex justify-between'>
    <div className="flex gap-5">
      <button onClick={() => window.history.back()} className="inline-flex items-center px-8 bg-[#6D0083]/80 text-white rounded-xl hover:scale-110 duration-300">
        ...
        {imageGroups.map((group, groupIndex) => (
          <div key={groupIndex} className='flex justify-center'>
            <div className='gap-10 inline-flex items-center text-xl'>
              <h3 className='flex justify-between w-[550px] border-4 border-[#6D0083] text-gray-200 p-4 mt-5 items-center rounded-xl font-bold'>{group.name}
                <span className="text-sm text-gray-400 ml-1">
                  ({group.images.length} módul{group.images.length !== 0 ? 'os' : ''})
                </span>
              </h3>
              <button onClick={() => openModal(group)} className='w-40 p-4 mt-5 bg-[#6D0083]/80 text-gray-200 rounded-xl font-mono hover:scale-110 duration-300'>Ver módulos</button>
              ...
              <div className='flex justify-center'>
                <div className="modal-buttons inline-flex mb-5">
                  <button
                    ...
                    className={`text-gray-200 mr-3 border-2 border-[#FFC805]/70 hover:bg-[#FFC805]/70 px-3 py-1 rounded-xl uppercase font-extrabold text-xl hover:scale-125 duration-300${currentImageIndex === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    &#60;
                  </button>
                  <div className="pagination flex font-extrabold">
                    {currentGroup.images.map((image, index) => (
                      <button
                        ...
                        className={`page-btn ${index === currentImageIndex ? 'bg-gray-300 text-[#FFC805]/70 rounded-xl' : 'bg-[#FFC805]/70 text-gray-200 hover:scale-125 duration-300 hover:bg-slate-200 hover:text-[#FFC805]/70'} px-4 py-2 rounded-xl`}>
                        {index + 1}
                      </button>
                    ))}
                  </div>
                  <button
                    ...
                    className={`text-gray-200 ml-3 border-2 border-[#FFC805]/70 hover:bg-[#FFC805]/70 px-3 py-1 rounded-xl uppercase font-extrabold text-xl hover:scale-125 duration-300${currentImageIndex === currentGroup.images.length - 1 ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    &#62;
                  </button>
                </div>
              </div>
  ...
  <div className="flex justify-center items-center mt-32">
    <button onClick={() => window.history.back()} className="text-center">
      <PacmanLoader color="#6D0083" size={80} className='w-60 text-gray-400'/>

...
import { openDatabase } from '../../../../components/modulos/moduleDigital/promas/database/index';
...
const convertToPDFAndEmail = async (group: { name: string, images: File[] }) => {
  try {
    ...
    const emailContent = render(<PromasTemplate groupName={group.name} dataString={dataString}/>)
    ...
    const mailOptions = {
      from: 'promascorporacion@gmail.com',
        ...
        {imageGroups.map((group, groupIndex) => (
          <div key={groupIndex} className='flex justify-center'>
            <div className='gap-10 inline-flex items-center text-xl'>
              <h3 className='flex justify-between w-[550px] border-4 border-[#FD4660] text-gray-200 p-4 mt-5 items-center rounded-xl font-bold'>{group.name}

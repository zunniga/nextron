...
import { openDatabase } from '../../../../components/modulos/moduleDigital/ecomas/database/index';
...
const ImageExport = () => {
  ...
  useEffect(() => {
    // Lógica para obtener datos y almacenarlos temporalmente
  ...
  const convertToPDFAndEmail = async (group: { name: string, images: File[] }) => {
    try {
      ...
      const emailContent = render(<EcomasTemplate groupName={group.name} dataString={dataString}/>)
      ...
      alert('Error al enviar el PDF por correooooo');
      setSendButtonState(prevState => ({
        ...prevState,
        [group.name]: 'initial'
      }));
  ...
  const sendEmailToAllGroups = async () => {
    try {
      ...
      for (const group of imageGroups) {
        ...
        await new Promise(resolve => setTimeout(resolve, 1000)); // Esperar 2 segundos
      }
        ...
        {imageGroups.map((group, groupIndex) => (
          <div key={groupIndex} className='flex justify-center'>
            <div className='gap-10 inline-flex items-center text-xl'>
              <h3 className='flex justify-between w-[550px] border-4 border-[#0060ff] text-gray-200 p-4 mt-5 items-center rounded-xl font-bold'>{group.name}

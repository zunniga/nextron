...
const CertificateGeneratorExcel = ({
 ...
 // IMAGEN DE ANVERSO
 // Función para ajustar dinámicamente el tamaño del texto
function ajustarTexto(ctx, texto, maxWidth, fontSizeInicial) {
  let fontSize = fontSizeInicial;
  ctx.font = `bold ${fontSize}px Century Gothic`;
  let textWidth = ctx.measureText(texto).width;

  // Reducir el tamaño de la fuente si el texto excede el ancho máximo permitido
...
const generateCertificate = async (participant, imageDataURL) => {
  if (imageDataURL) {
    ...
    // Ajustar el nombre del participante
    const fontSizeNombre = ajustarTexto(ctx, participant.nombreParticipante, 2600, 140);
    ctx.fillStyle = "#000000";
    ctx.textBaseline = "top";
    ctx.textAlign = "center";
    ctx.font = `bold ${fontSizeNombre}px Century Gothic`;
    ctx.fillText(participant.nombreParticipante, 2890, 1420);

    // Obtener el mes siguiente
    const words = participant.FechaFin.split(" ");
    const mesTexto = words[2].toLowerCase();
    const meses = ["enero", "febrero", "marzo", "abril", "mayo", "junio", "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"];
    const indice = meses.indexOf(mesTexto);
    let mesActual = (indice !== -1 && indice < meses.length - 1) ? meses[indice + 1] : "enero";
    let yearActual = new Date().getFullYear() + (indice === meses.length - 1 ? 1 : 0);

    // Dibujar la fecha
    const fechaTexto = `Lima, ${mesActual} del ${yearActual}`;
    ctx.font = "65px Century Gothic";
    ctx.fillText(fechaTexto, 3650, 2480);

    // Dibujar código del participante
    ctx.font = "bold 55px Century Gothic";
    ctx.fillText(participant.codigoParticipante, 700, 2875);

    // Dibujar el texto del diplomado
    const textoAntesCurso = `Por haber culminado y aprobado satisfactoriamente el DIPLOMADO DE ESPECIALIZACIÓN `;
    const textoDespuesCurso = ` en su calidad de asistente, aprobado mediante la ${participant.Resolucion}, llevado a cabo del ${participant.FechaInicio} al ${participant.FechaFin} con una duración de 420 horas académicas, equivalente a 26 créditos.`;
    const maxWidth = 2500;
    let y = 1680; // Posición inicial vertical
    ...
    function dividirTexto(ctx, texto, maxWidth) {
      ...
      palabras.forEach(palabra => {
        const nuevaLinea = lineaActual + (lineaActual === "" ? "" : " ") + palabra;
        const textWidth = ctx.measureText(nuevaLinea).width;
    ...
    // Dibujar el texto antes de `CursoName`
    ctx.font = "65px Century Gothic";
    const lineasAntes = dividirTexto(ctx, textoAntesCurso, maxWidth);
    lineasAntes.forEach(linea => {
      ctx.fillText(linea, 2870, y);
      y += 75; // Reducir espacio vertical entre las líneas
    });

    // Dibujar el texto de `CursoName` en negrita
    ctx.font = "bold 65px Century Gothic";
    const lineasCurso = dividirTexto(ctx, participant.CursoName, maxWidth);
    lineasCurso.forEach(linea => {
      ctx.fillText(linea, 2870, y);
      y += 75; // Reducir espacio vertical entre las líneas
    });

    // Dibujar el texto después de `CursoName`
    ctx.font = "65px Century Gothic";
    const lineasDespues = dividirTexto(ctx, textoDespuesCurso, maxWidth);
    ...
    // Generar el certificado como imagen
  ...
  const generateCertificateReverso = async (participant, imageDataURL) => {
    if (imageDataURL) {
      ...
      ctx.fillText(participant.CursoName, 3050, 1360)
      ctx.textAlign = "center"

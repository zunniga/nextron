...
import { NewDataCimade } from "./ImageCimadeDB" // Importa la clase ImageDatabase
...
const CertificateGenerator = () => {
  ...
  const loadSelectedImages = async () => {
    try {
      ...
      if (images.length > 0) {
        const digitalImage = images.find((image) => image.name === "imgAnverso")
        ...
        return new Promise((resolve, reject) => {
          ...
          img.onload = async () => {
            ...
            if (type === "certificadoDigital") {
              ...
              function ajustarTextoYAnchoMaximo(
                ctx,
                texto,
                maxWidth,
                fontSizeInicial
              ...
              const fontSizeFinal = ajustarTextoYAnchoMaximo(
                ctx,
                ParticipanteName,
                maxWidth,
                fontSizeInicial
              ...
              for (var i = 0; i < cantidadModulos; i++) {
                ...
                for (var palabra of palabras) {
                  var medidaPalabra = ctx.measureText(
                    linea + palabra + " "
                  ).width
      ...
      Promise.all(certificatesToSave)
        ...
        .then(() => {
          ...
          window.location.href =
            "/graduates_main/graduate_cimade/graduate/route/page"
        })
        .catch((error) => {
          console.error(

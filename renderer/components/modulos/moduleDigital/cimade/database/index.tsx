...
export const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise<IDBDatabase>((resolve, reject) => {
    if (db) {
      ...
      const request = window.indexedDB.open('ImageDatabaseCimade', 2);
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!dbInstance.objectStoreNames.contains('cimade')) {
          console.log('Creating object store: cimade');
          dbInstance.createObjectStore('cimade', { autoIncrement: true });
        }
        if (!dbInstance.objectStoreNames.contains('ImagesCimade')) {
          console.log('Creating object store: ImagesCimade');
          dbInstance.createObjectStore('ImagesCimade', { autoIncrement: true });
        }
      };
    ...
    const expectedObjectStores = ['cimade', 'ImagesCimade'];
...
const deleteAndRecreateDatabase = (): Promise<void> => {
  return new Promise<void>((resolve, reject) => {
    const dbName = 'ImageDatabaseCimade';

...
import { openDatabase }  from '../binex/database/index';
  ...
  useEffect(() => {
    const getStoredImages = async () => {
      try {
        ...
        const transaction = db.transaction(['binex'], 'readonly');
        const objectStore = transaction.objectStore('binex');
...
const handleImage = async (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const saveImages = async (images: File[]) => {
    try {
      const db = await openDatabase();
      const transaction = db.transaction(['binex'], 'readwrite');
      const objectStore = transaction.objectStore('binex');
      ...
      transaction.oncomplete = async () => {
        const storedImages: File[] = [];
        const newTransaction = db.transaction(['binex'], 'readonly');
        const newObjectStore = newTransaction.objectStore('binex');
...
useEffect(() => {
  ...
  return (
    <div className=''>
      ...
      <div className='flex justify-center border-b-4 border-[#2878B7] image-container relative mb-10 text-white font-mono'>
      ...
      {imagesToShow.map((file, index) => (
        <div key={index} className="image-container relative mb-4 flex justify-between items-center">
          {file && (
            <div className='border-2 border-[#2878B7] py-3 w-full mr-10 rounded-lg font-bold'>

...
export const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise<IDBDatabase>((resolve, reject) => {
    if (db) {
      ...
      const request = window.indexedDB.open('ImageDatabasePromas', 2);
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!dbInstance.objectStoreNames.contains('promas')) {
          console.log('Creating object store: promas');
          dbInstance.createObjectStore('promas', { autoIncrement: true });
        }
        if (!dbInstance.objectStoreNames.contains('ImagesPromas')) {
          console.log('Creating object store: ImagesPromas');
          dbInstance.createObjectStore('ImagesPromas', { autoIncrement: true });
        }
      };
    ...
    const expectedObjectStores = ['promas', 'ImagesPromas'];
...
const deleteAndRecreateDatabase = (): Promise<void> => {
  return new Promise<void>((resolve, reject) => {
    const dbName = 'ImageDatabasePromas';

...
import { openDatabase }  from '../sayan/database/index';
  ...
  useEffect(() => {
    const getStoredImages = async () => {
      try {
        ...
        const transaction = db.transaction(['sayan'], 'readonly');
        const objectStore = transaction.objectStore('sayan');
...
const handleImage = async (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const saveImages = async (images: File[]) => {
    try {
      const db = await openDatabase();
      const transaction = db.transaction(['sayan'], 'readwrite');
      const objectStore = transaction.objectStore('sayan');
      ...
      transaction.oncomplete = async () => {
        const storedImages: File[] = [];
        const newTransaction = db.transaction(['sayan'], 'readonly');
        const newObjectStore = newTransaction.objectStore('sayan');
    ...
    if (ctx) {
      ...
      image.onload = async () => {
        ...
        if (selectedData.actividadAcademica !== null) {
          ...
          ctx.fillText("Curso - taller organizado por Corporación SAYAN,", width, y);
...
useEffect(() => {
  ...
  return (
    <div className=''>
      <div className='inline-flex w-full justify-center items-center mb-10 p-4 font-bold text-2xl bg-[#12a9be]/80 text-white rounded-s-xl'>
        <FaRegImages className='text-4xl mr-2'/>
        <h1 >Cargar imagenes ({numModules})</h1>
      </div>
      <div className='flex justify-center border-b-4 border-[#12a9be]/80 image-container relative mb-10 text-white font-mono'>
        <input type='file' accept="image/*" onChange={handleImage} multiple className={`p-4 rounded-xl mb-10 bg-[#12a9be]/80 cursor-pointer hover:scale-110 duration-300 ${imagesLoaded ? 'opacity-50 pointer-events-none' : ''}`} />
      </div>
      {imagesToShow.map((file, index) => (
        <div key={index} className="image-container relative mb-4 flex justify-between items-center">
          {file && (
            <div className='border-2 border-[#12a9be]/80 py-3 w-full mr-10 rounded-lg font-bold'>

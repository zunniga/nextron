...
export const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise<IDBDatabase>((resolve, reject) => {
    if (db) {
      ...
      const request = window.indexedDB.open('ImageDatabaseRizo', 2);
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!dbInstance.objectStoreNames.contains('rizo')) {
          console.log('Creating object store: rizo');
          dbInstance.createObjectStore('rizo', { autoIncrement: true });
        }
        if (!dbInstance.objectStoreNames.contains('ImagesRizo')) {
          console.log('Creating object store: ImagesRizo');
          dbInstance.createObjectStore('ImagesRizo', { autoIncrement: true });
        }
      };
    ...
    const expectedObjectStores = ['rizo', 'ImagesRizo'];
...
const deleteAndRecreateDatabase = (): Promise<void> => {
  return new Promise<void>((resolve, reject) => {
    const dbName = 'ImageDatabaseRizo';

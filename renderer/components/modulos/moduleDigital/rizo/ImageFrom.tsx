...
import { openDatabase }  from '../rizo/database/index';
import { FaRegImages } from "react-icons/fa6";
import { BsImages } from "react-icons/bs";
import { ExcelData } from '../../interface/interface';
  ...
  useEffect(() => {
    const getStoredImages = async () => {
      try {
        ...
        const transaction = db.transaction(['rizo'], 'readonly');
        const objectStore = transaction.objectStore('rizo');
...
const handleImage = async (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const saveImages = async (images: File[]) => {
    try {
      const db = await openDatabase();
      const transaction = db.transaction(['rizo'], 'readwrite');
      const objectStore = transaction.objectStore('rizo');
      ...
      transaction.oncomplete = async () => {
        const storedImages: File[] = [];
        const newTransaction = db.transaction(['rizo'], 'readonly');
        const newObjectStore = newTransaction.objectStore('rizo');
    ...
    if (ctx) {
      ...
      image.onload = async () => {
        ...
        if (selectedData.actividadAcademica !== null) {
          ...
          ctx.fillText("Curso - taller organizado por Corporación RIZO,", width, y);
...
useEffect(() => {
  ...
  return (
    <div className=''>
      <div className='inline-flex w-full justify-center items-center mb-10 p-4 font-bold text-2xl bg-[#6D0083] text-white rounded-s-xl'>
        <FaRegImages className='text-4xl mr-2'/>
        <h1 >Cargar imagenes ({numModules})</h1>
      </div>
      <div className='flex justify-center border-b-4 border-[#6D0083] image-container relative mb-10 text-white font-mono'>
        <input type='file' accept="image/*" onChange={handleImage} multiple className={`p-4 rounded-xl mb-10 bg-[#6D0083] cursor-pointer hover:scale-110 duration-300 ${imagesLoaded ? 'opacity-50 pointer-events-none' : ''}`} />
      </div>
      {imagesToShow.map((file, index) => (
        <div key={index} className="image-container relative mb-4 flex justify-between items-center">
          {file && (
            <div className='border-2 border-[#FFC805]/70 py-3 w-full mr-10 rounded-lg font-bold'>

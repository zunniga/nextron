  ...
  const drawCanvas = async (canvas: HTMLCanvasElement, data: ExcelDataPhysic, dataIndex: number, arrayIndex: number) => {
    ...
    if (ctx) {
      ...
      image.onload = async () => {
        ...
        const maxWidthNombre = 1800; // Ancho máximo para el nombre
        const initialNombreFontSize = 72;
        ctx.font = `bold ${initialNombreFontSize}px Century Gothic`;
        // Verificar si el nombre excede el ancho máximo y ajustar el tamaño de la fuente si es necesario
        let nombreWidth = 0;
        ...
        // Configuración del ancho máximo para la actividad académica
        let actividadAcademicaMaxWidth = 2480; // Ancho máximo para la actividad académica si el nombre no excede el ancho máximo
        if (currentNombre !== null) {
            if (nombreWidth > maxWidthNombre) {
                actividadAcademicaMaxWidth = 1900; // Ancho máximo para la actividad académica si el nombre excede el ancho máximo
            }
        }
        // Configuración de la fuente para la actividad académica
        if (data.actividadAcademica !== null) {
            const actividadAcademicaWidth = ctx.measureText(data.actividadAcademica).width;
            let actividadAcademicaFontSize = 60; // Tamaño de fuente normal
          ...
          ctx.fillText("Curso - taller organizado por BINEX Educación Continua,", width, y);
...
if (excelData) {
    ...
    excelData.forEach((data) => {
      data.datos.forEach((row) => { // Iterar sobre cada objeto dentro de datos
        if (typeof row.nombres === 'string') {
          dataToSave.push(row.nombres);
          // console.log(`Nombre: ${row.nombres}`);
        }
      });
    });
    sessionStorage.setItem('nombresData', JSON.stringify(dataToSave));
    // console.log("Nombres guardados en sessionStorage:", sessionStorage.getItem('nombresData'));
  }
...
useEffect(() => {
  ...
  return (
    <div className=''>
      <div className='inline-flex w-full justify-center items-center mb-10 p-4 font-bold text-2xl bg-[#2878B7] text-white rounded-s-xl'>
        <FaRegImages className='text-4xl mr-2'/>
        <h1 >Cargar imagenes ({numModules})</h1>
      </div>
      <div className='flex justify-center border-b-4 border-[#00dbb8] image-container relative mb-10 text-white font-mono'>
        <input type='file' accept="image/*" onChange={handleImage} multiple className={`p-4 rounded-xl mb-10 bg-[#2878B7] cursor-pointer hover:scale-110 duration-300 ${imagesLoaded ? 'opacity-50 pointer-events-none' : ''}`} />
      </div>
      {imagesToShow.map((file, index) => (
        <div key={index} className="image-container relative mb-4 flex justify-between items-center">
          {file && (
            <div className='border-2 border-[#00dbb8] py-3 w-full mr-10 rounded-lg font-bold'>

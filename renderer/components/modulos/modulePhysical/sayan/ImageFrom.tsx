...
useEffect(() => {
  ...
  return (
    <div className=''>
      <div className='inline-flex w-full justify-center items-center mb-10 p-4 font-bold text-2xl bg-[#12a9be] text-white rounded-s-xl'>
      ...
      <div className='flex justify-center border-b-4 border-[#12a9be]/80 image-container relative mb-10 text-white font-mono'>
        <input type='file' accept="image/*" onChange={handleImage} multiple className={`p-4 rounded-xl mb-10 bg-[#12a9be] cursor-pointer hover:scale-110 duration-300 ${imagesLoaded ? 'opacity-50 pointer-events-none' : ''}`} />
      </div>
      {imagesToShow.map((file, index) => (
        <div key={index} className="image-container relative mb-4 flex justify-between items-center">
          {file && (
            <div className='border-2 border-[#12a9be] py-3 w-full mr-10 rounded-lg font-bold'>

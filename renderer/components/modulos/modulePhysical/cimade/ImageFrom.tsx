    ...
    if (ctx) {
      ...
      image.onload = async () => {
        ...
        if (data.actividadAcademica !== null) {
          ...
          ctx.fillText("Curso - taller organizado por CIMADE Educación Continua,", width, y);
    ...
    excelData && Object.keys(groupedData).forEach((nombre, index) => {
      groupedData[nombre].forEach(({ dataIndex, arrayIndex }) => {
        ...
        if (excelData[dataIndex]) {
          // Aquí asumo que drawCanvas ahora recibe ExcelDataPhysic en lugar de ExcelData
